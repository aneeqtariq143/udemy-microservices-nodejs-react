...
- **User generated data**: Data that is generated by the user. For example, the user uploads a profile picture, the user creates a post, the user creates a comment etc. **Persistent Volume** will be suitable to store the user generated data.
- **Intermediate data**: Data that is generated by the application. For example, the application generates a report, the application generates a log file etc. **Node Specific volume like emptyDir** will be suitable to store the intermediate data. 
###### Configure volume in kubernetes
![img_67.png](img_67.png)
###### Types of Volumes and drivers in kubernetes [Reference](https://kubernetes.io/docs/concepts/storage/volumes/#volume-types)
![img_70.png](img_70.png)
![img_71.png](img_71.png)
###### Example#1: emptyDir [reference](https://kubernetes.io/docs/concepts/storage/volumes/#emptydir)
- Pod that defines an emptyDir volume, the volume is created when the Pod is assigned to a node
- All containers in the Pod can read and write the same files in the emptyDir volume
- When a Pod is removed from a node for any reason, the data in the emptyDir is deleted permanently
- **Note**: A container crashing does not remove a Pod from a node. The data in an emptyDir volume is safe across container crashes.
- **Note**: Doesn't work with muplitple pods (replicaSet: 2 pr more)
###### Example#2: hostPath [reference](https://kubernetes.io/docs/concepts/storage/volumes/#hostpath)
- A hostPath volume mounts a file or directory from the host node's filesystem into your Pod.
- **Note**: Doesn't work with muplitple pods (replicaSet: 2 pr more) or multiple servers, because it's mounted on the host machine
###### Example#3: csi [reference](https://kubernetes.io/docs/concepts/storage/volumes/#csi)
- Container Storage Interface (CSI) is a specification that allows third-party storage providers to develop plugins that can be used with Kubernetes.
...
- **Note**: Access Modes: ReadWriteOnce, ReadOnlyMany, ReadWriteMany. These modes are used to specify how the volume can be mounted. For example, if we specify the `ReadWriteOnce` mode, then the volume can be mounted as read-write by a single node. If we specify the `ReadOnlyMany` mode, then the volume can be mounted as read-only by many nodes. If we specify the `ReadWriteMany` mode, then the volume can be mounted as read-write by many nodes. [Chart Reference Access Modes supported by different volume types](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#access-modes)
###### Persistent Volume vs Normal Volume
...
9. **Add new script in `auth` microservice `package.json` file**: Add the following script in the `auth` microservice `package.json` file.
    ...
    ```
10. **Deployment Options**: There are many deployment options available in GitHub Actions. Some of the popular deployment options are `Heroku`, `AWS`, `Google Cloud`, `Azure`, and `Kubernetes`.
    - **Heroku Deployment**: Heroku is a cloud platform that lets you build, deliver, monitor, and scale apps. We can deploy the application to Heroku using the `Heroku CLI`.
    - **AWS Deployment**: AWS is a cloud platform that provides a wide range of services. We can deploy the application to AWS using the `AWS CLI`.
    - **Google Cloud Deployment**: Google Cloud is a cloud platform that provides a wide range of services. We can deploy the application to Google Cloud using the `Google Cloud CLI`.
    - **Azure Deployment**: Azure is a cloud platform that provides a wide range of services. We can deploy the application to Azure using the `Azure CLI`.
    - **Kubernetes Deployment**: Kubernetes is a container orchestration platform that automates the deployment, scaling, and management of containerized applications. We can deploy the application to Kubernetes using the `kubectl` command.
    - **DigitalOcean Deployment**: DigitalOcean is a cloud platform that provides a wide range of services. We can deploy the application to DigitalOcean using the `DigitalOcean CLI`.
    - **Costing Example**:
    ![img_62.png](img_62.png)
11. **Create a Cluster in DigitalOcean**: Create a cluster in DigitalOcean by logging into the DigitalOcean account and creating a cluster.
    - **Creating Cluster using DigitalOcean CLI**: We can use the DigitalOcean CLI to create a cluster in DigitalOcean. 
      - **Install DigitalOcean CLI**: [installation guide](https://docs.digitalocean.com/reference/doctl)
      - **DigitalOcean CLI**: DigitalOcean CLI is a command-line interface for DigitalOcean. We can use the DigitalOcean CLI to create a cluster in DigitalOcean.
      - **Create a Cluster**: Run the `doctl kubernetes cluster create [cluster-name]` command to create a cluster in DigitalOcean.
      - **Get the Cluster Credentials**: Run the `doctl kubernetes cluster kubeconfig save [cluster-name]` command to get the cluster credentials.
      - **Verify the Cluster**: Run the `kubectl get nodes` command to verify the cluster.
12. **Debugging (Optional)**: We can configure locally installed `kubectl` to point to the DigitalOcean or Local cluster.
![img_63.png](img_63.png)
    - **Install DigitalOcean CLI**: [installation guide](https://docs.digitalocean.com/reference/doctl)
    - **Generate DigitalOcean Access Token**: Generate a DigitalOcean access token by logging into the DigitalOcean account and generating an access token.
    - **Authenticate DigitalOcean CLI**: Authenticate the DigitalOcean CLI using the `doctl auth init` command.
    - **Get the Cluster Credentials**: Run the `doctl kubernetes cluster kubeconfig save [cluster-name]` command to get the cluster credentials.
      - **Note**: by running the above command, it will automatically switch the `kubectl` context to the DigitalOcean cluster.
      - **Get kubectl Context**: Run the `kubectl config get-contexts` command to get the kubectl context.
      - **Manually Configure kubectl context (optional)**: Configure the kubectl context to point to the DigitalOcean cluster using the `kubectl config use-context [context-name]` command.
      - **Get Cloud Cluster List**: We can get the cluster name by running the `doctl kubernetes cluster list` command.
      - **Verify Cloud the Cluster**: Run the `kubectl get nodes` command to verify the cluster.
13. **Swap the `kubectl` context**: We can swap the `kubectl` context to the DigitalOcean cluster by running the `kubectl config use-context [context-name]` command.
    - **Get kubectl config**: Run the `kubectl config view` command to get the kubectl context.
    - **Get kubectl Context**: Run the `kubectl config get-contexts` command to get the kubectl context.
14. **Create a GitHub Action for Deployment**:
![img_64.png](img_64.png)
    - **Create a GitHub workflow**: Goto GitHub repository -> Actions -> Click `set up a workflow yourself` -> rename the file to `deploy-auth.yaml` because we're going to deploy the application -> Add the following code to the `deploy-auth.yaml` file.
      - **Note**: Update the `host` in `src/infra/k8s/ingress-srv.yaml` to point out the real domain name.
      - **Note**: In this example, we are imagining that we have a DigitalOcean cluster name `ticketing`. 
      - **Note**: We have to add the `DOCKER_USERNAME` secret to the GitHub repository. We can add the `KUBE_CONFIG` secret by going to the GitHub repository -> Settings -> Secrets -> New repository secret -> Add the `DOCKER_USERNAME` secret.
      - **Note**: We have to add the `DOCKER_PASSWORD` secret to the GitHub repository. We can add the `KUBE_CONFIG` secret by going to the GitHub repository -> Settings -> Secrets -> New repository secret -> Add the `DOCKER_PASSWORD` secret.
      - **Note**: We have to add the `DIGITALOCEAN_ACCESS_TOKEN` secret to the GitHub repository. We can add the `KUBE_CONFIG` secret by going to the GitHub repository -> Settings -> Secrets -> New repository secret -> Add the `DIGITALOCEAN_ACCESS_TOKEN` secret.
    ```yml
    name: deploy-auth # Name of the workflow to identify it in the GitHub Actions UI.

    on:
      push:
        branches:
          - main # Run the workflow on the main branch when code is pushed to the main branch.
        paths:
          - 'auth/**' # Run the workflow when code is pushed to the auth service.
    ...
    jobs:
      deploy:
        ...
        - run: cd auth && docker build -t aneeqtariq143/udemy-microservices-nodejs-react-auth-service . # Build the Docker image for the auth service.
        - run: docker login -u "$DOCKER_USERNAME" -p $DOCKER_PASSWORD # Login to the Docker Hub.
          env: # Set the environment variables.
            DOCKER_USERNAME: ${{ secrets.DOCKER_USERNAME }} # Get the DOCKER_USERNAME from the GitHub repository secrets.
            DOCKER_PASSWORD: ${{ secrets.DOCKER_PASSWORD }} # Get the DOCKER_PASSWORD from the GitHub repository secrets.
        - run: docker push aneeqtariq143/udemy-microservices-nodejs-react-auth-service # Push the Docker image to the Docker Hub.
        - run: digitalocean/action-doctl@v2 # Use the DigitalOcean action to deploy the application to the DigitalOcean cluster.
          with:
            token: ${{ secrets.DIGITALOCEAN_ACCESS_TOKEN }} # Get the DIGITALOCEAN_ACCESS_TOKEN from the GitHub repository secrets.
        - run: doctl kubernetes cluster kubeconfig save ticketing # Get the ticketing cluster credentials, replace ticketing with your cluster name.
        - run: kubectl rollout restart deployment auth-depl # Restart the auth deployment. replace auth-depl with your deployment name
    ```
    - **Commit the Changes**: Commit the changes to the GitHub repository.
    - **Push the Changes**: Push the changes to the GitHub repository.
    - **Verify the Deployment**: make some changes into auth microservice and push to the dev branch and then create a pull request to merge the changes into the main branch. After merging the changes into the main branch, the GitHub Actions will run the `deploy-auth` workflow and pushed the image to the docker hub.
    - **Deploy Kubernetes Manifests (Infrastructure Files)**: Goto GitHub repository -> Actions -> Click `set up a workflow yourself` -> rename the file to `deploy-manifests.yaml` because we're going to deploy the application -> Add the following code to the `deploy-manifests.yaml` file.
    ```yml
    name: deploy-manifests # Name of the workflow to identify it in the GitHub Actions UI.
    ...
    on:
      push:
        branches:
          - main # Run the workflow on the main branch when code is pushed to the main branch.
        paths:
          - 'infra/**' # Run the workflow when code is pushed to the auth service.
      ...
      deploy:
        ...
        - run: digitalocean/action-doctl@v2 # Use the DigitalOcean action to deploy the application to the DigitalOcean cluster.
          with:
            token: ${{ secrets.DIGITALOCEAN_ACCESS_TOKEN }} # Get the DIGITALOCEAN_ACCESS_TOKEN from the GitHub repository secrets.
        - run: doctl kubernetes cluster kubeconfig save ticketing # Get the ticketing cluster credentials, replace ticketing with your cluster name.
        - run: kubectl apply -f infra/k8s # Apply the Kubernetes manifests in the infra/k8s folder.
    ```
